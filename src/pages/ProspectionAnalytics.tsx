        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-900 p-12 rounded-[50px] shadow-3xl relative overflow-hidden">
           ...
           <div className="space-y-3 relative z-10">
              <h1 className="text-4xl font-black text-white tracking-tighter uppercase flex items-center gap-4">
                 Performance <span className="text-[#820AD1] italic">Hunter</span>
              </h1>
           ...
           <div className="flex gap-3 relative z-10">
              <Button 
                ...
                disabled={isHunting}
                className="h-14 px-10 rounded-2xl bg-[#6c08b0] hover:bg-emerald-700 text-white font-black uppercase text-[10px] tracking-widest shadow-2xl transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-50"
              >
                 {isHunting ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           <Card className="p-8 border-none shadow-xl rounded-[35px] bg-white group hover:scale-[1.02] transition-all">
              <div className="flex justify-between items-start mb-6">
                 <div className="w-14 h-14 bg-purple-50 rounded-2xl flex items-center justify-center text-[#820AD1] group-hover:bg-[#6c08b0] group-hover:text-white transition-colors shadow-sm">
                    <Search className="w-6 h-6" />
                 </div>
                 <Badge className="bg-purple-50 text-[#820AD1] border-none font-black text-[9px]">+12%</Badge>
              </div>
           ...
           <Card className="p-8 border-none shadow-xl rounded-[35px] bg-white group hover:scale-[1.02] transition-all">
              <div className="flex justify-between items-start mb-6">
                 ...
                 <Badge className="bg-purple-50 text-[#820AD1] border-none font-black text-[9px]">{Math.round((stats.enrichmentRate / stats.totalLeadsFound) * 100 || 0)}%</Badge>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                 <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                       <Users className="w-5 h-5 text-[#820AD1]" />
                    </div>
                    <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Últimos Alvos Identificados</h3>
                 </div>
                 <Badge className="bg-[#6c08b0] text-white border-none font-black text-[8px] uppercase tracking-widest px-3">Tempo Real</Badge>
              </div>
              <div className="p-2">
                       ...
                       {data?.latestLeads?.map((lead: any) => (
                          <div key={lead.id} className="p-6 bg-slate-50 rounded-[30px] flex items-center justify-between group hover:bg-white hover:shadow-xl transition-all border border-transparent hover:border-slate-100">
                             <div className="flex items-center gap-6">
                                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center font-black text-[#820AD1] shadow-sm uppercase group-hover:bg-[#6c08b0] group-hover:text-white transition-all">
                                   {lead.name.charAt(0)}
                                </div>
                                <div>
                                   ...
                                   <div className="flex items-center gap-3">
                                      ...
                                      <p className="text-[9px] font-bold text-[#820AD1] uppercase tracking-widest">Origem: {lead.source}</p>
                                   </div>
                                </div>
                             </div>
                             <div className="flex items-center gap-4">
                                {lead.extractedData && (
                                   <div className="flex gap-1">
                                      <Badge className="bg-emerald-100 text-[#820AD1] text-[8px] font-black uppercase tracking-tighter px-2 border-none">DADOS IA</Badge>
                                      <Badge className="bg-blue-100 text-[#820AD1] text-[8px] font-black uppercase tracking-tighter px-2 border-none">MAPS</Badge>
                                   </div>
                                )}
                                <Badge className={`text-[8px] font-black uppercase px-3 py-1.5 rounded-lg border-none ${
                                   ...
                                   lead.status === 'PROSPECTING' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-[#820AD1]'
                                }`}>
                                   {lead.status}
                                </Badge>
                             </div>
           ...
           <Card className="border-none shadow-2xl rounded-[45px] bg-slate-900 overflow-hidden">
              <div className="p-10 border-b border-white/5 flex items-center justify-between">
                 <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-[#820AD1]/20 rounded-xl flex items-center justify-center">
                       <Database className="w-5 h-5 text-[#820AD1]" />
                    </div>
                 ...
                 <div className="space-y-6">
                    {data?.logs?.map((log: any) => (
                       <div key={log.id} className="relative pl-8 border-l border-white/10 pb-2">
                          <div className={`absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full ${log.status === 'SUCCESS' ? 'bg-[#820AD1] shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]'}`} />
                          <div className="space-y-2">
                             <div className="flex items-center justify-between">
                                <p className="text-[10px] font-black text-white/50 uppercase tracking-widest">
                                   {format(new Date(log.createdAt), "HH:mm '•' dd MMM", { locale: ptBR })}
                                </p>
                                <Badge className={`text-[7px] font-black uppercase border-none ${log.status === 'SUCCESS' ? 'bg-[#820AD1]/10 text-[#820AD1]' : 'bg-red-500/10 text-red-500'}`}>
                                   {log.status}
                                </Badge>
                             </div>
                             ...
                             <div className="flex items-center gap-2 bg-white/5 p-3 rounded-xl border border-white/5">
                                <Search className="w-3 h-3 text-[#820AD1]" />
                                <p className="text-[10px] font-black text-[#820AD1]/80 uppercase truncate">"{log.query}"</p>
                             </div>
                             {log.icp && (
